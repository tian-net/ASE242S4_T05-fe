...
export default function CustomersPage() {
    ...
    const filtered = list.filter((c: any) => {
        ...
        const matchSearch = !s || c.firstName?.toLowerCase().includes(s) || c.lastName?.toLowerCase().includes(s) || c.docNum?.includes(s);
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
                <SearchBar value={search} onChange={setSearch} placeholder="Buscar nombre o doc..." />
                <FilterSelect label="Tipo Doc." value={docFilter} onChange={setDocFilter} options={[{ value: '', label: 'Todos' }, { value: 'DNI', label: 'DNI' }, { value: 'RUC', label: 'RUC' }, { value: 'CarnetExtranjeria', label: 'Carnet Ext.' }, { value: 'Pasaporte', label: 'Pasaporte' }]} />
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-200">
                <DataTable
                    headers={['Nombre', 'Email', 'Teléfono', 'Documento', 'Acciones']}
            ...
            <Modal open={modal.open} onClose={() => setModal({ open: false })} title={modal.edit ? 'Editar Cliente' : 'Nuevo Cliente'}>
                <div className="space-y-4">
                    ...
                    <Input label="Teléfono" type="tel" value={form.phone} onChange={e => { setForm(f => ({ ...f, phone: e.target.value })); validate('phone', e.target.value); }} error={errors.phone} />
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <Input label="N° Documento" value={form.docNum} onChange={e => { setForm(f => ({ ...f, docNum: e.target.value })); validate('docNum', e.target.value); }} error={errors.docNum} />
                    </div>
